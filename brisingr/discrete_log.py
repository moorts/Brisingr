...
def pohlig_hellman(g: int, h: int, p: int, factorize: Callable[[int], list[tuple[int,int]]]) -> int:
...
def order(g: int, p: int, N: int, factorize: Callable[[int], list[tuple[int,int]]]=None) -> int:
    """Find order in F(p)
    Args:
        g: element of F(p)
        p: order of F(p)
        N: order of multiplicative group (phi(p))
        factorize: [optional] factorization routine, for smooth numbers factorization is easy, therefore we can use knowledge of the factors to speed up calculating the order
...
print(garner([2,3,4], [3,7,16]))

print(order(g, p, p-1, naive_factorize))
